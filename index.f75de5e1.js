...
headers.forEach((header, index)=>{
    ...
    header.addEventListener("click", ()=>{
        const rows = [
            ...updatedTbody.querySelectorAll("tr")
        ];
        ...
        rows.sort((rowA, rowB)=>{
            ...
            if (header.textContent === "Age") return Number(cellA) - Number(cellB);
...
function createForm() {
    function createInput(nameIn, typeInput = "input") {
        ...
        function addAttributesForInput(input) {
            ...
            input.required = true;
            if (nameIn === "salary" || nameIn === "age") input.type = "number";
        }
    ...
    form.append(nameLabel, positionLabel, officeInput, ageLabel, salaryLabel, submitBtn);
...
function createUser(user) {
    ...
    for(const userProp in user){
        ...
        if (userProp === "userSalary") newCell.textContent = `$${Number(user[userProp]).toLocaleString()}`;
        else newCell.textContent = user[userProp];
...
button.addEventListener("click", (eventBtn)=>{
    ...
    newUser.userOffice = officesLocation[+document.querySelector('[name="office"]').value];
    ...
    for(const userProp in newUser)if (!newUser[userProp]) {
    ...
    if (newUser.userAge < 18 || newUser.userAge > 90 || newUser.userName.length < 4) fornInputsStatus = notificationTypes.warning.name;
    if (fornInputsStatus === notificationTypes.success.name) {
        clearAllInputs();
        createUser(newUser);
        createNotification(notificationTypes.success);
    } else createNotification(notificationTypes.warning);
});
